...
exports.saveInsertListKatalog = async (req, res) => {
    try {
        ...
        const querypabrik = await prisma.master_barang_pabrik_bahan_jadi.create({
            data: {
                ...commonData,
                keterangan: null,
                status: true
...
exports.saveUpdateStatusListKatalog = async (req, res) => {
    try {
        const idmm = Number(atob(req.params.idmm))
        const data = req.body
        const querymm = await prisma.master_barang_pabrik_bahan_jadi.updateMany({
            where: {
                mm_id: idmm
            },
            data: {
...
//=====================================Pemesanan KE BAHAN BAKU====================================

exports.getDaftarPenyedia = async (req, res) => {
    try {
        const pu = await prisma.master_barang_pabrik_bahan_baku.findMany({
            where: {
                profil_usaha: {
                    status: true
                }
...
const cekKatalog = async (idmm, idpu) => {
    const katalog = await prisma.master_barang_pabrik_bahan_jadi.findFirst({ where: { mm_id: idmm, profil_usaha_id: idpu } });
    return katalog;
}
